-- 0031: reveal every pick at the weekly lock instead of at each game's kickoff.
--
-- Previously a pick became visible at greatest(kickoff, anchor), so a Monday
-- night pick stayed hidden until Monday night. Now that 0030 freezes the whole
-- slate at the Sunday 1:00 ET anchor, hiding it protects nothing -- nobody can
-- change a pick in response. Revealing at the lock makes the full board visible
-- Sunday afternoon.
--
-- Reveal time = the week's lock anchor, or the game's own kickoff when the week
-- has no Sunday 1:00 slate (in that case picks lock per game, so a pick must
-- not be shown before its own game starts).
--
-- Early-week picks still stay hidden until the anchor: until then players can
-- still change their remaining picks, so seeing whether a Thursday pick hit
-- would be an edge. That was the original reason for the reveal delay and it
-- still holds.

drop policy if exists picks_select_after_reveal on public.picks;
  ...
  for select using (
    ...
    and exists (
      ...
      where g.id = picks.game_id
        and coalesce(public.week_lock_anchor(g.season, g.week), g.kickoff) <= now()
    )
  );
...
as $function$
  with anchors as (
    select w.week,
           public.week_lock_anchor(p_season, w.week) as lock_at
  ...
  pick_pts as (
    ...
    where p.league_id = p_league_id
      ...
      and coalesce(a.lock_at, g.kickoff) <= now()
...
drop function if exists public.week_reveal_anchor(int, int);
